...
#### `one_time_tokens`
Tokens generated by HR for employees to clock in.

| Column | Type | Notes |
|--------|------|-------|
| id | INT AUTO_INCREMENT | Primary key |
| generated_by | INT | FK → users(id) — the HR user |
| for_user_id | INT | FK → users(id) — the employee |
| token_hash | VARCHAR(255) | SHA-256 hash |
| plain_token | VARCHAR(20) | 8-char alphanumeric (stored for HR display) |
| token_type | ENUM('regular','overtime') | |
| expires_at | TIMESTAMP | |
...
| `TOKEN_EXPIRY_MINUTES` | `60` | How long one-time tokens are valid |
