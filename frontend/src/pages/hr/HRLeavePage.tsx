    ...
    <>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Review, approve and manage employee leave requests</p>
        </div>
                  ...
                  <div className="px-4 py-4 flex flex-wrap items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className="font-semibold text-gray-900">{lr.employee_name}</span>
                        <span className="text-xs text-gray-400">{lr.department_name}</span>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5 line-clamp-2">{lr.description}</p>
                      {lr.hr_note && (
                    ...
                    <div className="flex flex-wrap gap-2 shrink-0">
                      ...
                      {(lr.extensions || []).length > 0 && (
                        <button onClick={() => toggleExpand(lr.id)} className="flex items-center gap-1 text-xs text-gray-500 hover:text-gray-700">
                  ...
                  {isExpanded && (lr.extensions || []).length > 0 && (
                    <div className="px-4 pb-4 border-t border-gray-50 pt-3 space-y-2">
                      ...
                      {lr.extensions!.map(ext => (
                        <div key={ext.id} className="flex flex-wrap items-start justify-between gap-2 bg-gray-50 rounded-lg px-3 py-2">
        ...
        >
          <form onSubmit={handleReview} className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-3 text-sm space-y-1">
              <p><span className="text-gray-500">Employee:</span> <strong>{reviewModal.employee_name}</strong></p>
              <p><span className="text-gray-500">Leave type:</span> {LEAVE_LABELS[reviewModal.leave_type]}</p>
              <p><span className="text-gray-500">Dates:</span> {format(new Date(reviewModal.start_date), 'd MMM')} → {format(new Date(reviewModal.end_date), 'd MMM yyyy')} ({reviewModal.days_requested} days)</p>
              <p className="text-gray-500 text-xs mt-1">{reviewModal.description}</p>
            </div>
            <div>
        ...
        <Modal title="Review Extension Request" onClose={() => setExtModal(null)} size="sm">
          <form onSubmit={handleExtReview} className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-3 text-sm">
              ...
              <p className="text-gray-500 text-xs mt-1">{extModal.reason}</p>
            </div>
