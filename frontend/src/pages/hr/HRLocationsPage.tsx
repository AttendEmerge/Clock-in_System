    ...
    <>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Acceptable Locations</h1>
            <p className="text-gray-500 text-sm mt-1">Manage approved clock-in locations</p>
          </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Coordinates</th>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Radius</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {locations.map(loc => (
                      <tr key={loc.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-gray-500">{loc.radius_meters}m</td>
                        <td className="px-4 py-3 text-gray-500">{loc.added_by_name || '—'}</td>
                        <td className="px-4 py-3">
      ...
      {(createModal || editModal) && (
        <Modal title={editModal ? 'Edit Location' : 'Add Acceptable Location'} onClose={() => { setCreateModal(false); setEditModal(null); }} size="lg">
          <form onSubmit={editModal ? handleEdit : handleCreate} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Click on the map to set location, or enter coordinates manually</label>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Acceptable Radius: {formRadius}m</label>
              <input
                type="range"
                min={50}
                max={2000}
              ...
              <p className="text-xs text-gray-400 mt-1">Employees must be within this radius to clock in without flagging</p>
            </div>
            <button type="submit"
